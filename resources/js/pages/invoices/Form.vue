<!--
  ...
  明細行ごとに税率を設定できる（標準10%・軽減8%・非課税0%）。
  小計・消費税・合計はリアルタイムで自動計算される。

  ルート:
    /invoices/create    → 新規作成
    /invoices/:id/edit  → 編集
-->
<template>
  <div class="max-w-4xl space-y-4">
    ...
    <div class="bg-white rounded-lg border shadow-sm p-6">
      <div class="flex justify-between items-center mb-4">
        ...
        <!-- 明細行追加 -->
        <button type="button"
          @click="form.items.push({ item_name: '', unit_price: 0, quantity: 1, unit: '式', amount: 0, tax_rate: '10' })"
        ...
        <tbody>
          <tr v-for="(item, i) in form.items" :key="i" class="border-b border-gray-100">
            <td class="py-1.5 pr-2">
              <input v-model="item.item_name" type="text" class="input" />
            </td>
            <td class="py-1.5 pr-2">
              <!-- 単価変更時に金額を再計算 -->
      ...
      <div class="mt-4 border-t pt-4 flex flex-col items-end gap-1 text-sm">
        <div class="flex justify-between w-52">
          <span class="text-gray-600">小計</span><span>{{ fmt(subtotal) }}</span>
        </div>
        <div class="flex justify-between w-52">
          <!-- 行ごとに税率が異なるため、消費税は各行で計算して合算 -->
          <span class="text-gray-600">消費税</span><span>{{ fmt(tax) }}</span>
        </div>
    ...
    <div class="flex gap-3">
      ...
      <a href="#/invoices" class="btn-secondary">キャンセル</a>
    </div>
  </div>
</template>
...
const form = ref({
  ...
  due_date:     '',
  notes:        '',
  items: [{ item_name: '', unit_price: 0, quantity: 1, unit: '式', amount: 0, tax_rate: '10' }],
})

/** 小計: 全明細 amount の合計 */
const subtotal = computed(() => form.value.items.reduce((s, i) => s + (i.amount ?? 0), 0))
/** 消費税: 行ごとの税率で計算して合算（切り捨て） */
const tax = computed(() =>
  Math.floor(form.value.items.reduce((s, i) => s + (i.amount ?? 0) * (parseInt(i.tax_rate ?? 0) / 100), 0))
)
/** 税込合計 */
...
onMounted(async () => {
  ...
  if (id) {
    const s = await api.get(`/invoices/${id}`)
...
async function submit() {
  ...
  try {
    if (id) {
      ...
      flash.success('請求書を更新しました。')
    } else {
      await api.post('/invoices', form.value)
      flash.success('請求書を作成しました。')
    }
