<!--
  ...
  明細行（items）を動的に追加できる。
  勘定科目は費用（expense）カテゴリのもののみ表示する。
  合計金額はリアルタイムで自動計算される。

  ルート:
    /expenses/create    → 新規申請
    /expenses/:id/edit  → 編集（draft 状態のみ）
-->
<template>
  <div class="max-w-3xl space-y-4">
    ...
    <div class="bg-white rounded-lg border shadow-sm p-6">
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input v-model="form.title" type="text" class="input" placeholder="1月 交通費精算" />
        </div>
        <div>
    ...
    <div class="bg-white rounded-lg border shadow-sm p-6">
      <div class="flex justify-between items-center mb-4">
        ...
        <!-- 明細行追加 -->
        <button type="button"
          @click="form.items.push({ account_item_id: '', item_date: '', description: '', amount: 0, tax_rate: '10' })"
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-200 text-xs text-gray-500">
            <th class="text-left pb-2 pr-2">日付</th>
            <th class="text-left pb-2 pr-2">内容</th>
            <th class="text-left pb-2 pr-2">勘定科目</th>
            <th class="text-right pb-2 pr-2 w-32">金額</th>
            <th class="text-center pb-2 w-16">税率</th>
            <th class="w-6" />
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, i) in form.items" :key="i" class="border-b border-gray-100">
            ...
            <td class="py-1.5 pr-2 w-36">
              <input v-model="item.item_date" type="date" class="input text-xs" />
            </td>
            <!-- 内容・摘要 -->
            <td class="py-1.5 pr-2">
              <input v-model="item.description" type="text" class="input text-xs" placeholder="電車代 渋谷→新宿" />
            </td>
            <!-- 勘定科目（費用科目のみ） -->
            <td class="py-1.5 pr-2 w-40">
              <select v-model="item.account_item_id" class="input text-xs">
                <option value="">選択</option>
                <option v-for="a in accounts" :key="a.id" :value="a.id">{{ a.name }}</option>
              </select>
            </td>
            <td class="py-1.5 pr-2">
              <input v-model.number="item.amount" type="number" class="input text-right" min="1" />
            </td>
            <!-- 税率（軽減税率対応） -->
            <td class="py-1.5 pr-2 text-center">
              <select v-model="item.tax_rate" class="input text-xs">
            ...
            <td class="py-1.5 text-center">
              <button type="button" @click="form.items.splice(i, 1)"
                v-if="form.items.length > 1" class="text-red-400 text-xs">✕</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <!-- 合計金額 -->
      <div class="mt-4 border-t pt-3 flex justify-end text-sm font-bold">
        <span class="mr-4 text-gray-600">合計</span>
...
onMounted(async () => {
  ...
  // 費用科目のみフィルタリング
  accounts.value = r.data.filter(a => a.category === 'expense')
  if (id) {
    const s = await api.get(`/expenses/${id}`)
...
async function submit() {
  ...
  try {
    if (id) {
      ...
      flash.success('経費を更新しました。')
    } else {
      await api.post('/expenses', form.value)
      flash.success('経費を申請しました。')
    }
